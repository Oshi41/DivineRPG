...
public class RenderDragonEntity extends RenderLiving<DivineDragonBase> {
    private final ResourceLocation dragon;
    private final ResourceLocation explode;
    private final ResourceLocation beam;

    public RenderDragonEntity(RenderManager renderManagerIn) {
        this(renderManagerIn,
                new ResourceLocation("textures/entity/enderdragon/dragon.png"),
                new ResourceLocation("textures/entity/enderdragon/dragon_exploding.png"),
    ...
    public RenderDragonEntity(RenderManager renderManagerIn, ResourceLocation dragon, ResourceLocation explode, ResourceLocation beam) {
        ...
        this.beam = beam;
        this.addLayer(new LayerDivineDragonEyes(this));
        this.addLayer(new LayerDivineDragonDeath());
    }

    protected void applyRotations(DivineDragonBase entityLiving, float p_77043_2_, float rotationYaw, float partialTicks) {
     ...
     */
    protected void renderModel(DivineDragonBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
        if (entitylivingbaseIn.deathTicks > 0) {
            float f = (float) entitylivingbaseIn.deathTicks / 200.0F;
            GlStateManager.depthFunc(515);
            GlStateManager.enableAlpha();
            GlStateManager.alphaFunc(516, f);
            this.bindTexture(explode);
     ...
     */
    public void doRender(DivineDragonBase entity, double x, double y, double z, float entityYaw, float partialTicks) {
        super.doRender(entity, x, y, z, entityYaw, partialTicks);

        BlockPos position = entity.getHealingPosition();

        if (position != null) {
            this.bindTexture(beam);
            float f = MathHelper.sin(((float) entity.ticksExisted + partialTicks) * 0.2F) / 2.0F + 0.5F;
            f = (f * f + f) * 0.2F;
            renderCrystalBeams(x, y, z, partialTicks, entity.posX + (entity.prevPosX - entity.posX) * (double) (1.0F - partialTicks), entity.posY + (entity.prevPosY - entity.posY) * (double) (1.0F - partialTicks), entity.posZ + (entity.prevPosZ - entity.posZ) * (double) (1.0F - partialTicks), entity.ticksExisted, position.getX(), (double) f + position.getY(), position.getZ());
        }
    }
     ...
     */
    protected ResourceLocation getEntityTexture(DivineDragonBase entity) {
        return dragon;
    }
}
